...
const AdminRoom = () => {
  ...
  const fetchRoom = async () => {
    ...
    const handleChangeStatus = async(id) => {
        const res = await axios.put(`/change-status/${id}`);
                ...
                <tr className="hover:bg-gray-50 cursor-pointer" key={e._id} onClick={() => handleRedirect(e._id)}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-end gap-4">
                              {
                                  !e.status && (<button className="bg-blue-500 hover:bg-blue-700 text-white font-light py-2 px-4 rounded-full" onClick={() => handleChangeStatus(e._id)}>Duyệt</button>)
                      }
                    </div>
                  </td>
                </tr>
